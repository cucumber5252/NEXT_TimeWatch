...
import styles from './ChatUrl.module.css';
...
export default function ChatUrl({ url }) {
    ...
    const [isProfanityFilterOn, setIsProfanityFilterOn] = useState(false); // 필터 상태 기본값: OFF
    ...
    const fetchMessages = async () => {
        if (!url) {
            // 콘솔 에러 숨김, 채팅 잘 표시되는데도 에러 뜸
            // console.error('URL is not defined');
            return;
        }
        try {
            const response = await axios.get(`/api/chats/${encodeURIComponent(url)}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`/api/chats/${encodeURIComponent(url)}`, {
                username: userName,
            ...
            fetchMessages(); // 메시지를 전송한 후 전체 메시지 다시 가져오기
        } catch (error) {
    ...
    const handleDelete = async (messageId) => {
        ...
        try {
            const response = await axios.delete(`/api/chats/${encodeURIComponent(url)}`, {
    ...
    const handleRefreshMessages = async () => {
        ...
        if (img) {
            ...
            img.style.animation = 'spin 1s linear';
        }
    ...
    return (
        <div className={styles.chatUrlContainer}>
            <div className={styles.chatContainer}>
                ...
                <form onSubmit={handleSubmit} className={styles.form}>
                    ...
                    <button type="submit" className={styles.sendButton}>
                        <Image src="/send.svg" alt="Send" width={22} height={12} />
                    </button>
                </form>
            </div>
        </div>
