    ...
    Runner.prototype = {
        ...
        init: function () {
            ...
            if (IS_MOBILE) {
                ...
                // 공룡 터치 영역 추가 및 위치 설정
                if (this.tRexTouchArea) {
                    this.containerEl.appendChild(this.tRexTouchArea);
                    this.updateTRexTouchArea();

                    // 화면 크기가 변할 때마다 위치 업데이트
                    window.addEventListener(Runner.events.RESIZE, this.updateTRexTouchArea.bind(this));
                }
            }
        ...
        updateTRexTouchArea: function () {
            if (!this.tRex || !this.tRexTouchArea) return;

            // 게임이 진행 중이거나(인트로 아닐 때), 재시작된 경우 터치 영역 활성화
            const isGameActive =
                (this.playing && !this.playingIntro) || // 게임 진행 중
                this.restarted || // 재시작된 경우
                this.crashed; // 게임 종료 상태

            if (isGameActive) {
                this.tRexTouchArea.style.display = 'block';

                // 캔버스 내에서의 공룡 위치 계산
                const canvasRect = this.canvas.getBoundingClientRect();

                // 공룡 좌표는 게임 내 논리적 위치이므로 실제 화면에 맞게 조정
                const scale = canvasRect.width / this.dimensions.WIDTH;
                const tRexXPos = this.tRex.xPos * scale;
                const tRexYPos = this.tRex.yPos * scale;
                const tRexWidth = (this.tRex.ducking ? this.tRex.config.WIDTH_DUCK : this.tRex.config.WIDTH) * scale;
                const tRexHeight = this.tRex.config.HEIGHT * scale;

                // 터치 영역을 공룡보다 약간 크게 설정 (터치하기 쉽게)
                const padding = 20 * scale;

                this.tRexTouchArea.style.left = tRexXPos - padding / 2 + 'px';
                this.tRexTouchArea.style.top = tRexYPos - padding / 2 + 'px';
                this.tRexTouchArea.style.width = tRexWidth + padding + 'px';
                this.tRexTouchArea.style.height = tRexHeight + padding + 'px';

                // 디버그용으로 터치영역 표시 (주석 해제하여 사용)
                // this.tRexTouchArea.style.backgroundColor = 'rgba(255, 0, 0, 0.3)';
            } else {
                // 게임 시작 전에는 터치 영역 숨김
                this.tRexTouchArea.style.display = 'none';
            }
        },
        ...
        createTouchController: function () {
            ...
            // 캐릭터 터치 영역은 공룡이 그려진 후에 위치를 계산해야 하므로
            // 여기서는 영역만 생성하고, init 완료 후 위치 조정
            this.tRexTouchArea = document.createElement('div');
            this.tRexTouchArea.className = 't-rex-touch-area';
            this.tRexTouchArea.style.position = 'absolute';
            this.tRexTouchArea.style.zIndex = '10';
            // this.tRexTouchArea.style.backgroundColor = 'rgba(255, 0, 0, 0.3)'; // 디버깅용

            // 캐릭터 터치 영역은 init 완료 후 추가
            // this.containerEl.appendChild(this.tRexTouchArea);
        },
        ...
        adjustDimensions: function () {
            ...
            if (this.canvas) {
                ...
                // 추가: 화면 크기가 변경되면 공룡 터치 영역도 업데이트
                if (IS_MOBILE && this.tRexTouchArea) {
                    this.updateTRexTouchArea();
                }
            }
        },
            ...
            if (this.playing) {
                ...
                if (!collision) {
                    ...
                    if (this.playing && !this.crashed) {
                        if (!this.lastScoreUpdate || now - this.lastScoreUpdate > 100) {
                            // 점수 계산을 distanceMeter와 동일하게 맞춤
                            const displayScore = this.distanceMeter.getActualDistance(Math.ceil(this.distanceRan));
        ...
        startListening: function () {
            ...
            if (IS_MOBILE) {
                // 화면 전체는 점프
                this.touchController.addEventListener(Runner.events.TOUCHSTART, this.onJumpTouch.bind(this));
                this.touchController.addEventListener(Runner.events.TOUCHEND, this.onJumpEnd.bind(this));

                // 캐릭터 영역은 숙이기
                if (this.tRexTouchArea) {
                    this.tRexTouchArea.addEventListener(Runner.events.TOUCHSTART, this.onDuckTouch.bind(this));
                    this.tRexTouchArea.addEventListener(Runner.events.TOUCHEND, this.onDuckEnd.bind(this));
                }

                // 게임 재시작을 위한 리스너 (기존 코드)
         ...
         * 점프 터치 처리 핸들러
         */
        onJumpTouch: function (e) {
            e.preventDefault();

            // 이미 숙이기가 처리되었으면 이벤트 전파 중지
            if (e.tRexTouchProcessed) {
                return;
            }
         ...
         * 숙이기 터치 처리 핸들러
         */
        onDuckTouch: function (e) {
            e.preventDefault();
            e.stopPropagation(); // 이벤트가 상위(점프)로 전파되지 않도록

            // 이벤트 처리 플래그 설정
            e.tRexTouchProcessed = true;
         ...
         * 숙이기 종료 처리 핸들러
         */
        onDuckEnd: function (e) {
            e.preventDefault();
            e.stopPropagation();

            // 게임이 진행 중이고 인트로가 끝난 상태에서만 숙이기 종료 처리
            if (this.playing && !this.playingIntro && !this.crashed) {
        ...
        stopListening: function () {
            ...
            if (IS_MOBILE) {
                this.touchController.removeEventListener(Runner.events.TOUCHSTART, this.onJumpTouch);
                this.touchController.removeEventListener(Runner.events.TOUCHEND, this.onJumpEnd);

                if (this.tRexTouchArea) {
                    this.tRexTouchArea.removeEventListener(Runner.events.TOUCHSTART, this.onDuckTouch);
                    this.tRexTouchArea.removeEventListener(Runner.events.TOUCHEND, this.onDuckEnd);
                }
        ...
        gameOver: function () {
            ...
            // 중요: 게임 화면에 표시되는 것과 동일한 점수 계산법 사용
            const finalScore = this.distanceMeter.getActualDistance(Math.ceil(this.distanceRan));
            // console.log('게임 오버, 전송되는 최종 점수:', finalScore);
